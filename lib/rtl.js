...
var wrapLambda = $asyncscript.wrapLambda = function(f, signature, binding){
	var result;
...
invisibleField(Array.prototype, "__$asrtl_getmember", function(member){
	if(member = arrayMethods[member]) return member;
		...
		case "tail": return this.slice(1);
		case "head": return this.slice(0, this.length - 2);
	}
	if($asyncscript.state.checked) throw runtimeErrors.missingMember;
	else return null;
});
...
invisibleField(Function, "__$asrtl_invoke", function(args){
	var body = args[0];
	if(args.length < 2) return runtimeErrors.invalidArgCount;
	var sig = new Signature(args[1]);
	sig.__$size$__ = args.length - 2;
	for(var i = 2, contract; i < args.length; i++)
		if((contract = args[i]).__$asrtl_relationship) sig[i - 2] = contract;
