...
Promise.prototype.complete = function(err, result){
	function proceed(){
		function eventHandler(handler){ handler(this.result); };
		if(this.isError) this.events.error.forEach(eventHandler, this);
		else this.events.success.forEach(eventHandler, this);
...
/**
 * Creates a new promise using the specified contract.
 */
$asyncscript.newPromise = function(contract){
	if(contract instanceof Property) return this.newPromise(contract.value);
	else if(contract instanceof Promise)
		if(contract.isError) throw contract.result;
		else if(contract.isCompleted) return this.newPromise(contract.result);
		else return this.fork(arguments.callee.bind(this), contract);
	else if("__$asrtl_relationship" in contract || contract === null || contract === undefined) return new Promise(contract);
	else if(this.state.checked) throw runtimeErrors.contractExpected;
	else return null;
};
...
var runtimeErrors = $asyncscript.errors = {
	...
	undeclaredId: function(name, filename, position){
		filename = filename && position ? "(" + filename + ":" + position.column + ":" + position.line + ") " : "";
		return position ? this.create('ASRUNTIME_UNDECLARED_NAME', filename + 'Identifier ' + name + ' was not declared') : htis.create('Identifier ' + name + ' was not declared');
	}
};
...
RuntimeState.prototype.pushLayer = function(globals){ return this.layers.push(globals || {}); };
RuntimeState.prototype.popLayer = function(){ return this.layers.pop(); };
RuntimeState.prototype.inLayers = function(name){
	for(var i = this.layers.length - 1; i >= 0; i--)
		if(name in this.layers[i]) return true;
	return false;
};
RuntimeState.prototype.fromLayer = function(name){
	for(var i = this.layers.length - 1; i >= 0; i--){
		var globals = this.layers[i];
		if(name in globals) return globals.__$asrtl_getmember instanceof Function ? globals.__$asrtl_getmember(name) : globals[name];
	}
};
 ...
 */
var newLambda = $asyncscript.newLambda = function(implementation){
	...
	switch(arguments.length){
		...
		return function(arg0, arg1){
			...
			return destination;
		}.toLambda(arguments[1], arguments[2]);
		//generic lambda
		default:
		signature = new Array(arguments.length - 1);
		for(var i = 1; i < arguments.length; i++) signature[i - 1] = arguments[i];
		return Function.prototype.toLambda.apply(function(){
			var destination = $asyncscript.enterLambdaBody();
...
function Expression(tree, deserialized){
	...
	this.tree = deserialized ? tree : (tree = ast.deserialize(tree));
...
function OverloadList(){
	...
	for(var i = 0; i < arguments.length; i++){
		...
		if(f instanceof OverloadList)
			for(var j = 0; j < f.__$size$__; j++)
				this[idx++] = f[j];
		else if(f instanceof Array) f.forEach(function(f){ this[idx++] = f; }, this);
		else this[idx++] = f;
	}
...
OverloadList.prototype.toString = function(){
	...
	for(var i = 0; i < this.__$size$__; i++){
		result += '(' + toScriptString(this[i]) + ')';
		if(i !== this.__$size$__ - 1) result += " + ";
	}
	return result;
};
...
function Signature(){
	for(var i = 0; i < arguments.length; i++){
		...
		this[i] = contract === undefined ? Object : contract;	
	}
	invisibleField(this, "__$size$__", arguments.length, true);
}
...
function wrapLambda(f, signature, binding){
	var result;
	...
	Object.defineProperty(result, "__$contract$__", {value: signature});
...
invisibleField(Function.prototype, "toLambda", function(){
	...
	//wraps lambda function and overrides signature
		...
		var signature;
		if(arguments.length === 0) signature = Signature.empty;
		else if(arguments[0] instanceof Signature) signature = arguments[0];
		else{
			(signature = new Signature()).__$size$__ = arguments.length;
			for(var i = 0; i < arguments.length; i++) signature[i] = (arguments[i] !== undefined) ? arguments[i] : Object;
 ...
 */
function Property(getter, setter, contract, _this){
	Object.defineProperty(this, "this", {writable: false, value: _this});
	...
	if(contract === undefined || contract === Object){
		...
		Object.defineProperty(this, "value", {
			get: getter instanceof Function ? getter : throwWriteonly,
			set: setter instanceof Function ? setter : empty
		});
	}
	else {
		Object.defineProperty(this, "__$contract$__", {writable: false, value: contract});
		Object.defineProperty(this, "value", {
			get: getter instanceof Function ? function(){
				return $asyncscript.binding(this.getter.call(this["this"]), this["__$contract$__"]);
			} : throwWriteonly,
			set: setter instanceof Function ? function(value){
...
Property.prototype = {
	bind: function(_this){
		return new Property(this.getter, this.setter, this["__$contract$__"], false, _this);	
	},
	get draft(){ return this["this"] === undefined; },
...
invisibleField(Boolean.prototype, "__$asrtl_typecast", function(type){
	switch(type){
		...
		case Function: return thisProvider.bind(this).toLambda();
		default:
...
invisibleField(RegExp.prototype, '__$asrtl_typecast', function(right){
	switch(right){
		...
		case Function: return thisProvider.bind(this).toLambda();
		default:
			if(right instanceof Signature) return thisProvider.bind(this).toLambda();
			else if(type instanceof FilterContract) return type.typecast(this);
			else if($asyncscript.state.checked) throw runtimeErrors.unsupportedOp('typecast');
			else return null;
	}
...
invisibleField(Array.prototype, "__$asrtl_typecast", function(right){
	switch(right){
		...
		default:
			...
			else if(type instanceof Signature) return thisProvider.bind(this).toLambda();
			else if(type instanceof FilterContract) return type.typecast(this);
			if($asyncscript.state.checked) throw runtimeErrors.unsupportedOp('typecast');
			else return null;
	}
...
OverloadList.prototype.__$asrtl_binary_equ = function(right){
	if(right instanceof OverloadList && right.__$size$__ === this.__$size$__){
		for(var i = 0; i < this.__$size$__; i++){
			var result = false;
			for(var j = 0; j < this.__$size$__; i++)
				if(this[i] === right[i]) {result = true; break;}
	...
	switch(right){
		...
		default:
			if(right instanceof Signature){
				var result;
				for(var i = 0; i < this.__$size$__; i++)
					switch($asyncscript.relationship(this[i].__$contract$__, right)){
						case "equal": return this[i];
						case "subset":
							if(!result) result = this[i];
						default: continue;
					}
...
$asyncscript.getMember = function(obj, member){
	...
	else if(obj instanceof Object && member in obj) return obj[member]; 
...
var exprMethods = {
	isQuoted: newLambda(function(expr){ return (expr && expr.tree.quoted) ? true : false }, Expression),
	...
	}, Expression, new Signature(Expression)),
	//compiles the node
	...
	}, Expression, $asyncscript.integer)
};
...
[
	...
	this[e.name] = newLambda(new Function("expr", "return expr && expr.tree.nodeType === " + JSON.stringify(e.nodeType)), Expression);
}, exprMethods);
...
Property.prototype.__$asrtl_getmember = function(member){ return $asyncscript.getMember(this.value, member); };

var typedefMethods = {
	isFilter: newLambda(function(contract){ return contract instanceof FilterContract; }, Object),
	filter: newLambda(function(filter){
		return filter && filter.__$contract$__ instanceof Signature && filter.__$contract$__["__$size$__"] > 0 ?
			new FilterContract(filter.__$contract$__[0], filter.toLambda(Object)) : null;
	}, Function)
};
...
var stringMethods = {
	indexOf: newLambda(String.prototype.indexOf, String),
	substr: newLambda(String.prototype.substr, $asyncscript.integer, $asyncscript.integer),
	cmp: newLambda(String.prototype.localeCompare, String),
	...
	charCode: newLambda(String.prototype.charCodeAt, $asyncscript.integer)
};
...
var lambdaMethods = {
	...
	}, Object, containerContract.empty, Object),
...
var regexpMethods = {
	test: newLambda(RegExp.prototype.test, String)
};
...
$asyncscript.assignment = function(left, right){
	return left instanceof Property ? (left.value = right, left) : right;
};
...
function(idx){
	if(isInteger(idx)) return this[idx];
...
OverloadList.prototype.__$asrtl_invoke = function(args){
	//creates a signature from the arguments
	var required = Signature.fromArguments.apply(Signature, args);
	//find matching function
	for(var i = 0; i < this.__$size$__; i++){
		var fn = this[i];
		//check the count of arguments
		if(fn.__$contract$__["__$size$__"] > args.length) continue;
		switch($asyncscript.relationship(fn.__$contract$__, required)){
			case "equal":
			case "superset": return fn.apply(null, args);
			default: continue;
		}
	}
	return new Error("Overloaded lambda function cannot be called with the specified arguments.");	
};
...
$asyncscript.invokeMethod = function(_this, method, args, destination){
	...
	else if(_this instanceof Property) return this.invokeMethod(_this.value, method, args, destination);
	else if(_this instanceof Promise)
		if(_this.isError) throw _this.result;
...
$asyncscript.invokeAsync = function(_this, f, args, destination){
	if(destination === undefined) args.push(this.createCallback(destination = new Promise()));
	else if(destination instanceof Promise) args.push(this.createCallback(destination));
	return this.invoke(_this, f, args), destination;
};
...
/**
 * Resolves the specified identifier at runtime.
 * @param {String} name The name of the identifier to resolve.
 */
$asyncscript.resolveName = function(name, position){
	if(name in this.globals) return this.globals[name];
	else if(name in global) return global[name];
	var value = this.fromLayer(name);
	if(value !== undefined) return value;
	else throw runtimeErrors.undeclaredId(name, position);
};

$asyncscript.awaitArgument = function(args, index, _this){
...
$asyncscript.globals = require('./globals.js');
