...
SyntaxAnalyzer.prototype.parseTree = function(expr, check, terminators, priority, callback){
	...
	switch(this.lookup){
		...
		//new promise value
		case Lexeme.keywords['async']:
			return expr ? callback(ParserError.expectedOperator(this.lookup, column, line)) :
				ast.CodeAsyncExpression.parse(this, column, line, terminators, subtreeCallback(false));
		//expansion
		case Lexeme.keywords['expandq']:
			return expr ? callback(ParserError.expectedOperator(this.lookup, column, line)) :
				ast.CodeExpansionExpression.parse(this, column, line, subtreeCallback(true)); 
		//with expression
		case Lexeme.keywords['with']:
			return expr ? callback(ParserError.expectedOperator(this.lookup, column, line)) :
				ast.CodeWithExpression.parse(this, column, line, terminators, subtreeCallback(false));
		//loop expression
		...
		//parse fork
		case Lexeme.keywords['fork']:
			return expr ? callback(ParserError.expectedOperator(this.lookup, column, line)) :
				ast.CodeForkExpression.parse(this, column, line, terminators, subtreeCallback(false));
		//parse context
		...
		//parse await
		case Lexeme.keywords['await']:
			return expr ? callback(ParserError.expectedOperator(this.lookup, column, line)) :
				ast.CodeAwaitExpression.parse(this, column, line, terminators, subtreeCallback(false));
		case Lexeme.punctuation['{']:
			return expr ? callback(ParserError.expectedOperator(this.lookup, column, line)) :
		...
		default: switch(this.lookup.kind){ //Attempts to parse by lexeme kind
			...
			case 'nmtoken':
				...
				else if(expr) return callback(ParserError.expectedOperator(this.lookup, column, line));
