...
/**
 * Creates a new FORK expression.
 * @param {Object} column The number of the column in the source code.
 * @param {Integer} line The number of the line in the source code.
 */
function CodeForkExpression(column, line){
	if(arguments.length === 0) return;	//deserialization case
...
CodeForkExpression.prototype.toString = function(){
	return '(fork ' + this.target + ')';
};

CodeForkExpression.parse = function(parser, column, line, terminators, callback){
	var result = new CodeForkExpression(column, line);
	var expr = parser.shouldNextLexeme();
	if(expr instanceof ParserError) return callback(expr);
	parser.next(terminators, function(err, target){
		if(err) return callback(err);
		this.target = target;
...
CodeAwaitExpression.parse = function(parser, column, line, terminators, callback){
	var result = new this(column, line);
	//passes through await keyword
	var expr = parser.nextLexeme(
	...
	async.asyncWhile(function(callback){ return callback(this.lookup !== Lexeme.punctuation[')']); }.bind(parser),
		function(next){
			var expr = parser.shouldNextLexeme();
			if(expr instanceof ParserError) return callback(expr);
			parser.next([Lexeme.punctuation[','], Lexeme.punctuation[')']], function(err, expr){
				if(err) return callback(err);
				else if(!(expr instanceof CodeLetExpression)) return callback(ParserError.expected('let', expr, parser.column, parser.line));
				this.synchronizedValues.push(expr);
				return next();
			}.bind(this));
		}.bind(result),
		function(){
			...
			else if(expr !== Lexeme.punctuation['->']) return callback(ParserError.expected('->', expr, parser.column, parser.line));
			...
			parser.next(terminators, function(err, body){
				...
				switch(parser.lookup){
					case Lexeme.punctuation[':']:
						expr = parser.shouldNextLexeme();	//pass through else keyword
						if(expr instanceof ParserError) return callback(expr);
						return parser.next(terminators, function(err, _else){
							if(err) return callback(err);
							this['else'] = _else;
...
function CodeCurrentFunctionExpression(column, line, quoted){
	if(arguments.length === 0) return;	//deserialization case
	this.nodeType = "CodeCurrentFunctionExpression";
	this.position = {'column': column, 'line': line};
	this.quoted = quoted;
}

CodeCurrentFunctionExpression.prototype.toString = function(){
	return this.quoted ? '@@' : '@';
};
...
CodeFunctionExpression.prototype.toString = function(){
	var result = '(' + (this.quoted ? '@@' : '@') + this.parameters;
...
CodeFunctionExpression.parse = function(parser, column, line, terminators, callback){
	...
	var result = new this(column, line);
	result.quoted = parser.lookup === Lexeme.punctuation['@@'];
	var lex = parser.shouldNextLexeme();
	if(lex instanceof ParserError) return callback(lex);
	...
	else return callback(undefined, new CodeCurrentFunctionExpression(result.position.column, result.position.line, result.quoted));
};
...
function CodeAsyncExpression(column, line){
	if(arguments.length === 0) return;	//deserialization case
	this.nodeType = "CodeAsyncExpression";
	this.position = {'column': column, 'line': line};
}

CodeAsyncExpression.prototype.toString = function(){
	return '(async ' + this.contract + ')';
};

CodeAsyncExpression.parse = function(parser, column, line, terminators, callback){
	var lex = parser.shouldNextLexeme();
	if(lex instanceof ParserError) return callback(lex);
	parser.next(terminators, function(err, contract){
		if(err) return callback(err);
...
module.exports = {
	...
	'CodeForkExpression': CodeForkExpression,
	'CodeAwaitExpression': CodeAwaitExpression,
	...
	'CodeCurrentFunctionExpression': CodeCurrentFunctionExpression,
	...
	'CodeAsyncExpression': CodeAsyncExpression,
	...
	restore: function(tree, result){
		if("nodeType" in tree && tree.nodeType in this)
			result = new this[tree.nodeType]();	//creates instance of the syntax tree
		...
		else if(tree instanceof Array) return tree.map(function(e){ return this.restore(e, result); }, this);
		else result = {};
		//analyze each key
		Object.keys(tree).forEach(function(key){
			result[key] = this.restore(tree[key]);
		}, this);
		return result;
	},
