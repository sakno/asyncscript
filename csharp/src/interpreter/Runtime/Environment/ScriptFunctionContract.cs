...
namespace DynamicScript.Runtime.Environment
{
    ...
    [ComVisible(false)]
    [Serializable]
    public class ScriptFunctionContract : ScriptContract, ISerializable, IEquatable<ScriptFunctionContract>
    {
        #region Nested Types
        ...
        /// <summary>
        /// Represents collection of the aggregated slots.
        /// </summary>
        private static readonly AggregatedSlotCollection<ScriptFunctionContract> StaticSlots = new AggregatedSlotCollection<ScriptFunctionContract>
        {
            {"ret", (owner, state) => owner.ReturnValueContract}
        };
        ...
        private ScriptFunctionContract(Signature sig, bool copy = false)
        {
        ...
        protected ScriptFunctionContract(SerializationInfo info, StreamingContext context)
        ...
        public ScriptFunctionContract(IEnumerable<Parameter> parameters, IScriptContract returnValue = null)
        ...
        /// <param name="returnValue">The contract of the return value. Can be <see langword="null"/> if action doens't return any value.</param>
        public ScriptFunctionContract(IEnumerable<KeyValuePair<ParameterExpression, IScriptContract>> parameters, IScriptContract returnValue = null)
        ...
        internal static NewExpression New(NewArrayExpression signature, Expression @return)
        {
            ...
            var ctor = LinqHelpers.BodyOf<IEnumerable<Parameter>, IScriptContract, ScriptFunctionContract, NewExpression>((ps, c) => new ScriptFunctionContract(ps, c));
        ...
        public bool Equals(ScriptFunctionContract other)
        {
            return other != null && 
        ...
        public bool IsComposable(ScriptFunctionContract contract)
        {
        ...
        public sealed override IScriptObject Convert(IScriptObject value, InterpreterState state)
        {
            if (Mapping(ref value))
                ...
                {
                    case ContractRelationshipType.Superset:
                    case ContractRelationshipType.TheSame: return ScriptRuntimeFunction.CreateConstantLambda(value);
                    default: break;
                }
        ...
        public ContractRelationshipType GetRelationship(ScriptFunctionContract other)
        {
        ...
        {
            if (contract is ScriptFunctionContract)
                return GetRelationship((ScriptFunctionContract)contract);
        ...
        {
            return ScriptRuntimeFunction.CreateEmptyImplementation(this);
        }
        ...
        /// <summary>
        /// Gets or sets value of the indexed object.
        /// </summary>
        ...
        public sealed override IScriptObject this[IList<IScriptObject> indicies, InterpreterState state]
        {
            get
            {
                if (indicies.Count == 1)
                {
                    var paramName = indicies[0] as ScriptString;
                    if (paramName != null)
                    {
                        var p = GetParameterByName(paramName);
                        if (p != null)
                            return p.ContractBinding;
                        else if (state.Context == InterpretationContext.Unchecked)
                            return Void;
                        else throw new SlotNotFoundException(paramName, state);
                    }
                    else throw new ContractBindingException(indicies[0], ScriptStringContract.Instance, state);
                }
        ...
        /// <returns></returns>
        public ScriptFunctionContract RemoveParameter(string parameterName)
        {
            switch (m_signature.Contains(parameterName))
            {
                case true:
                    var result = new ScriptFunctionContract(m_signature, true);
