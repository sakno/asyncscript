...
namespace DynamicScript.Runtime.Environment.Threading
{
    ...
    sealed class ScriptWorkItemQueue: IScriptWorkItemQueue
    {
        ...
        {
            return new ScriptWorkItemState(m_implementation[ScriptNativeQueue.EnqueueActionName, state].Invoke(new[] { new ScriptWorkItemAction(target, workItem) }, state), state);
        }
    }
}
