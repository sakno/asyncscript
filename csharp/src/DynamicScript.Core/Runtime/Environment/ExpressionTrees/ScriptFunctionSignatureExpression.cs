...
namespace DynamicScript.Runtime.Environment.ExpressionTrees
{
    ...
    [ComVisible(false)]
    [Serializable]
    sealed class ScriptFunctionSignatureExpression : ScriptExpression<ScriptCodeActionContractExpression, IScriptContract>
    {
        private ScriptFunctionSignatureExpression(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
        }

        public ScriptFunctionSignatureExpression(ScriptCodeActionContractExpression expression)
            : base(expression, ScriptFunctionSignatureExpressionFactory.Instance)
        {
        }
